...
class UtilityCurve():
    ...
    def __init__( self, x_0, x_25, x_50, x_75, x_100
        ...
        be used to prevent recalculating UtilityCurve function at runtime.
        """
        self.true_min = x_0
        self.true_max = x_100
        x_0   = self.normalize(  x_0) # if not invert else 1.0 /   x_0)
        x_25  = self.normalize( x_25) # if not invert else 1.0 /  x_25)
        x_50  = self.normalize( x_50) # if not invert else 1.0 /  x_50)
        x_75  = self.normalize( x_75) # if not invert else 1.0 /  x_75)
        x_100 = self.normalize(x_100) # if not invert else 1.0 / x_100)
...
if __name__=="__main__":
    ...
    commute_time = UtilityCurve(     50,     40,     20,     10,       5, invert=True)
    salary       = UtilityCurve(  17000,  40000,  73000, 107000,  160000, invert=False)
    home_price   = UtilityCurve( 415000, 501000, 600000, 815000, 1200000, invert=False)
    work_week    = UtilityCurve(     60,     50,     45,     40,      37, invert=True)

    options = [ commute_time
              , salary
              , home_price
              , work_week
              ]
    map(lambda x: x.setWeight(1.0/len(options)), options)

    print sum([ commute_time(30)
              , salary(73350 + 12 * (1200 + 500))
              , home_price(592198)
              , work_week(37)
              ])

    #for point_set in getMSR_ND(options, N = 30):
    #    print ','.join(map(lambda (uc, x): str(uc(x)), zip(options, point_set)))
    #    #print ','.join(map(str, point_set))
